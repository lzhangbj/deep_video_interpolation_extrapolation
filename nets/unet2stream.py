...
class UNet2Stream(nn.Module):
	...
	def forward(self, input, gt=None):
		encoded_feature = self.encoder(input)
		transformed_feature = self.dil_conv(encoded_feature)
		shared_feature = self.decoder_head(transformed_feature)
		output_rgb = F.tanh(self.rgb_decoder(shared_feature))
		output_seg = self.seg_decoder(shared_feature)
