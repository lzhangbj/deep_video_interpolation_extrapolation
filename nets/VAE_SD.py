...
class VAE_SD(nn.Module):
	def __init__(self, args):
		super(VAE_SD, self).__init__()

		self.args = args
		self.seg_encoder = SegEncoder(in_dim=20, out_dim=4)

		# BG
		self.flow_encoder_bg = FlowEncoder(args, in_dim=(3+args.seg_dim)*(args.vid_length+1))
		# FG
		self.flow_encoder_fg = FlowEncoder(args, in_dim=(3+args.seg_dim)*(args.vid_length+1))

		self.encoder = encoder(args)
		self.flow_decoder = decoder(args)


		self.zconv = convbase(256+64, 16*self.args.vid_length, 3, 1, 1)
		...
		self.refine = True
		if self.refine:
			self.refine_net = RefineNet(num_channels=3+20)
	...
	def forward(self, rgb_data, seg_data, bg_mask, fg_mask, noise_bg, z_m=None):
		...
		input = torch.cat([frame1, seg_encoded[:, 0]], 1)
		...
		if z_m is None:
			...
			mu_bg, logvar_bg = self.flow_encoder_bg(
				torch.cat(
						[y, bg_seg_encoded.view(-1, args.seg_dim * (args.vid_length+1), args.input_size[0], args.input_size[1])], 1)
				.contiguous())
			# FG
			mu_fg, logvar_fg = self.flow_encoder_fg(
				torch.cat(
						[y, fg_seg_encoded.view(-1, args.seg_dim * (args.vid_length+1), args.input_size[0], args.input_size[1])], 1)
		...
		codey = self.zconv(torch.cat([self.fc(z_m).view(-1, 64, int(args.input_size[0]/16), int(args.input_size[1]/16)), codex], 1)) # 64, 8, 8
		codex = torch.unsqueeze(codex, 2).repeat(1, 1, args.vid_length, 1, 1)  # bs,256,4,8,8
		codey = torch.cat(torch.chunk(codey.unsqueeze(2), args.vid_length, 1), 2)  # bs,16,4,8,8
		...
		output_seg = warp(seg1, flow, args, self.floww, mask_fw)

		y_pred = output

		'''Go through the refine network.'''
		if self.refine:
			y_pred = refine(torch.cat([output,output_seg], dim=2), flow, mask_fw, self.refine_net, args, noise_bg)
			y_pred_seg = y_pred[:, :, 3:]
			y_pred = y_pred[:,:,:3]
		...
		if self.training:
			...
			return output, y_pred, output_seg, y_pred_seg, mu, logvar, flow, flowback, mask_fw, mask_bw, prediction_vgg_feature, gt_vgg_feature
		else:
			return output, y_pred, output_seg, y_pred_seg, flow, flowback, mask_fw, mask_bw
